...
class ProbSymmetrizedPolicy(nn.Module):
    ...
    def get_action_and_log_prob(self, obs, return_angles=False):
        ...
        a = dist.rsample().view(S, B, -1)
        logp = dist.log_prob(a.view(S * B, -1)).sum(-1, keepdim=True).view(S, B, 1)

        inv_R = R.transpose(-1, -2)
        def undo(idx, do):
            blk = a[..., idx:idx + 2]
            return torch.einsum('sbji,sbj->sbi', inv_R, blk) if do else blk
        ...
        parts = [undo(0, r12)]
        if a.size(-1) >= 4:
            parts.append(undo(2, r34))
        if a.size(-1) >= 6:
            parts.append(undo(4, r56))
    ...
    for ep in range(args.num_episodes):
        ...
        while not ts.last():
            ...
            if len(replay) >= args.batch_size and global_step % args.update_every == 0 and global_step >= args.learning_starts:
                ...
                actor_loss = (args.alpha * lp - q_new).mean() - 0.01 * ang_ent  # 0.1
